...
class ImageOnlyModel(nn.Module):
    def __init__(self, cnn_backbone, cnn_output_dim=512, hidden_dim=128):
        ...
        self.combined_fc = nn.Sequential(
            nn.Linear(cnn_output_dim, hidden_dim),  # hidden_dim = 128
            nn.ReLU(),
            nn.Dropout(0.3), #increased dropout from 0.2 to 0.3 to reduce overfitting
            nn.Linear(hidden_dim, hidden_dim // 2),
            nn.ReLU(),
            nn.Dropout(0.2),# increased dropout from 0.1 to 0.2 to reduce overfitting
